...
```{r}
boxplot(len ~ supp, data=my_data, frame = FALSE, 
...
> **Respuesta:** El boxplot sugiere que la mediana de la longitud de los dientes aumenta a medida que aumenta la dosis de vitamina C. La variabilidad parece ser similar en los tres grupos de dosis.
...
```{r}
boxplot(len ~ dose * supp, data=my_data, frame = FALSE, 
        col = c("#00AFBB", "#E7B800"), ylab="Tooth Length")
```
...
```{r}
interaction.plot(x.factor = my_data$supp, trace.factor = my_data$dose, 
                 response = my_data$len, fun = mean, 
                 type = "b", legend = TRUE, 
                 xlab = "Supp", ylab="Tooth Length",
                 pch=c(1,19), col = c("#00AFBB", "#E7B800"))
```
...
> **Respuesta:** La prueba de Shapiro-Wilk no rechaza la hipótesis nula de normalidad (p = 0.06), lo que sugiere que los residuos siguen una distribución normal.
...
> **Respuesta:** La prueba de Durbin-Watson no rechaza la hipótesis nula de independencia (p = 0.43), lo que sugiere que los residuos son independientes.
...
> **Respuesta:** La prueba de Levene no rechaza la hipótesis nula de homogeneidad de varianzas (p = 0.74), lo que sugiere que las varianzas de los residuos son homogéneas.
...
> **Respuesta:** El tamaño del efecto de la dosis de vitamina C (dose) es grande (η² = 0.79), lo que indica que la dosis de vitamina C explica una gran proporción de la varianza en la longitud de los dientes. El tamaño del efecto del método de entrega (supp) es moderado (η² = 0.34), lo que indica que el método de entrega explica una proporción moderada de la varianza en la longitud de los dientes. El tamaño del efecto de la interacción entre el método de entrega y la dosis es pequeño (η² = 0.06), lo que indica que la interacción explica una pequeña proporción de la varianza en la longitud de los dientes.
...
> **Respuesta:** La potencia obtenida para el factor "supp" es de 0.84, lo que indica que la prueba tiene una alta probabilidad de detectar un efecto significativo del método de entrega si realmente existe.
...
```{r}
power.anova.test(groups = 3, n = 20, between.var = 542.2, within.var = 712.1, sig.level = 0.05)
```

> **Respuesta:** La potencia obtenida para el factor "dose" es de 0.99, lo que indica que la prueba tiene una probabilidad muy alta de detectar un efecto significativo de la dosis si realmente existe.
