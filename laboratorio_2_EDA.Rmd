...
```{r}
# Al usar choose() usted puede elegir el archivo sin necesidad de definir la ubicación
# definir header y encodinmg le ayudan a read.csv a identificar cabeceras y el formato del texto
df= (read.csv('pokemon.csv', header=T, encoding = "UTF-8"))
attach(df)
...
```{r}
density_plot <- function(data, column , fill_color, color, transparency, title){
  ggplot(data=data, aes(column)) + geom_density(col=color,fill=fill_color, alpha=transparency) + ggtitle(title)
}
```
...
density_hp <- ggplot(data=df, aes(hp)) + geom_density(col="white",fill="pink", alpha=0.8) + ggtitle("Diagrama de densidad de Hit Points o Vida")
...
density_height <- ggplot(data=df, aes(height_m)) + geom_density(col="white", fill="tan3", alpha=0.7) + ggtitle("Diagrama densidad de caracteristicas ofensivas")

density_weight <- density_plot(data=df, column=weight_kg, fill_color="gray", color='white', transparency=0.8, title="Diagrama densidad de caracteristicas peso")

grid.arrange(density_hp, density_speed, density_attack, density_defense, density_weight, density_height, ncol=2)
```

```{r}
ggplot(data=df, aes(type1)) + geom_bar(aes(fill=..count..), alpha=0.8) + theme(axis.text.x =element_text(angle = 90, hjust = 0)) + ggtitle("Distribucion basados en Type-1") + coord_flip()

ggplot(data=df, aes(type2)) + geom_bar(aes(fill=..count..), alpha=0.8) + theme(axis.text.x =element_text(angle = 90, hjust = 0)) + ggtitle("Distribucion basados en Type-2") + coord_flip()
...
scale_color_gradient(low="darkblue", high="red") + ggtitle("Contraste Caracteristicas de
Defensa vs Ataque") +
...
```{r}
ggplot(na.omit(df), aes(attack, speed)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, (attack > 170 | attack < 50 & speed >150 | speed < 50) & is_legendary == 1 | speed > 145), aes(label=name),box.padding = 0.35, point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method =
"lm") + labs(x = 'Attack', y = 'Speed')

ggplot(na.omit(df), aes(attack, weight_kg)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, (attack > 170 | attack
< 50 | weight_kg > 650) & (is_legendary == 1)), aes(label=name), box.padding = 0.35,
point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method = "lm") + labs(x = 'Attack', y = 'Weight')

ggplot(na.omit(df), aes(attack, height_m)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, ((attack > 170 | attack
< 50 | height_m > 7.5) & is_legendary == 1) | height_m > 5 & is_legendary == 0),
aes(label=name), box.padding = 0.35, point.padding = 0.5, segment.color = 'grey50') +
geom_smooth(method = "lm") + labs(x = 'Attack', y = 'Height')

ggplot(na.omit(df), aes(attack, hp)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, ((attack > 170 | hp >
190 | attack < 50) & is_legendary == 1) | hp >160), aes(label=name), box.padding = 0.35,
point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method = "lm") + labs(x = 'Attack', y = 'HP')
```

```{r}
speed_defense_legendary <- ggplot(na.omit(df), aes(defense, speed)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, (defense > 170 | defense
< 50 & speed >150 | speed < 50) & is_legendary == 1 | speed > 145), aes(label=name),
box.padding = 0.35, point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method =
"lm")

weight_defense_legendary <- ggplot(na.omit(df), aes(defense, weight_kg)) +
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, (defense > 170 | defense
< 50 | weight_kg > 650) & (is_legendary == 1)), aes(label=name), box.padding = 0.35,
point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method = "lm")
...
geom_point(aes(color=is_legendary)) + geom_label_repel(data=subset(df, ((defense > 170 | hp > 190 | defense < 50) & is_legendary == 1) | hp >160), aes(label=name), box.padding = 0.35, point.padding = 0.5, segment.color = 'grey50') + geom_smooth(method = "lm")

grid.arrange(speed_defense_legendary, weight_defense_legendary, height_defense_legendary,
hp_defense_legendary, ncol = 2)
```

```{r}
box_plot_attr <- select(df, type1, is_legendary, hp, defense, attack, sp_attack, sp_defense,
speed)
...
geom_boxplot(fill="green4") + ggtitle("Pokemon Legendario")
bp_nor <- ggplot(data = box_plot_attr_nor_long, aes(attribute, value)) + geom_boxplot(fill
= "yellow2") + ggtitle("Pokemon Normal")
...
ggplot(data=hmap_attr_leg_m, aes(type1, variable)) + geom_tile(aes(fill=value)) +
ggtitle("Legendary Pokemon: Type1 - Attribute") + scale_fill_gradientn(colours =
hm.palette(100)) + theme(axis.text.x = element_text(angle=90, hjust=0)) + coord_equal()
```
...
hmap_attr_nor <- summarise(hmap_attr_nor, hp=median(hp), attack=median(attack),
defense=median(defense), sp_attack=median(sp_attack), sp_defense=median(sp_defense),
speed=median(speed))
...
ggplot(data=hmap_attr_nor_m, aes(type1, variable)) + geom_tile(aes(fill=value)) +
ggtitle("Normal Pokemon: Type1 - Attribute") + scale_fill_gradientn(colours =
hm.palette(100)) + theme(axis.text.x = element_text(angle=90, hjust=0)) + coord_equal()
```
...
ggtitle("Correlación de Atributos - Legendarios") + scale_fill_gradientn(colours =
hm.palette(100)) + coord_equal() + labs (x = '', y = '')
```
...
ggplot(data=hmap_attr_nor_cor_m, aes(Var1, Var2)) + geom_tile(aes(fill=value)) +
ggtitle("Correlación de Atributos - Normales") + scale_fill_gradientn(colours =
hm.palette(100)) + coord_equal() + labs (x = '', y = '')
```
...
theme(axis.text.x=element_text(angle=90, hjust=0)) + ggtitle("Efectividad por tipo de
Pokemon")
```
